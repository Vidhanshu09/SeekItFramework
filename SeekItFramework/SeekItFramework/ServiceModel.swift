...
open class ServiceModel: Equatable {
    ...
    weak var serviceModelManager: SensorServiceModelManager?
     ...
     */
    public func writeValue(withUUID uuid: String, response: Bool = true) {
    ...
    public func writeValue(withUUID uuid: String, response: Bool = true, complition:@escaping (_ charecterstic: CBCharacteristic) -> Void){
        let value = getValueInServiceModel(withUUID: uuid)
        guard let dataTransformer = transformer(forUUID: uuid) else {
            return
        }
        let data = dataTransformer.transform(valueToData: value)
        serviceModelManager?.setNotificationHandler(complition: complition)
        serviceModelManager?.writeValue(data, toCharacteristicUUID: uuid, serviceUUID: serviceUUID, response: response)
    }
